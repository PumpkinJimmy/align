...
int main(int argc, char** argv)
{
	...
	while (1)
	{
		...
		else { diffs.push_back(diff); cout << ox << ' ' << oy << endl; cnt++;};
		writer << img;
	...
	aver = sum / areas.size();
	for (double d: diffs) diff_mean += d;
	diff_mean /= diffs.size();
	for (double d: diffs) diff_var += (d - diff_mean) * (d - diff_mean);
	diff_var /= diffs.size();
	cout << "Diff mean: " << diff_mean << endl << "Diff std: " << sqrt(diff_var) << endl;

	for (double a: areas) var += (a - aver) * (a - aver);
	var /= areas.size();

	cout << "Mean: " << aver << endl << "Varient: " << sqrt(var) << endl;
	return 0;
}
